...
- **User**: the default user is still `abc` (webtop's own), *not* renamed to
  ...
  other webtop image.
- **Dotfiles installed three times**: into `/root` (so `sudo`/`su -` gets
  the same shell), into `/config` (`abc`'s home), and into `/etc/skel` (so
  any additional user created later - manually, or by a future multi-user
  desktop setup - gets the same bundle via `useradd -m` for free).
...
- **One in-cluster-kubeconfig hook**: [`packaging/custom-cont-init.d/10-kubeconfig.sh`](packaging/custom-cont-init.d/10-kubeconfig.sh)
  runs once at container start (linuxserver's supported one-shot extension
  point - `/custom-cont-init.d`, *not* `/config/custom-services.d`, which is
  reserved for long-running services and isn't scanned under `/config`
  anyway) and generates an in-cluster kubeconfig for `abc` if a
